...
http://your.server.com/~user/path-project/hub.txt" and you should be
able to paste that URL into [[http://genome.ucsc.edu/cgi-bin/hgHubConnect#unlistedHubs][My Data -> Track Hubs -> My Hubs]] then
click Add Hub to tell the UCSC genome browser to display your data.
Navigate around the genome until you have found some peaks, then add
labels for them in =project/labels/*.txt= files.
